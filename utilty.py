...
def apply_SR(model,input_image,scale):

    if len(input_image.shape) >= 3 and input_image.shape[2] == 3:
        input_y_image = convert_rgb_to_y(input_image)
        input_y_image=tf.expand_dims(input_y_image,axis=0)
        input_y_image=tf.image.convert_image_dtype(input_y_image, tf.float32)/255.
        output_y_image =model(input_y_image)
        #output_y_image =  tf.squeeze(output_y_image)
        y_bic=convert_rgb_to_y(resize_image_by_pil(input_image, scale))
        output_y_image=np.add(output_y_image.numpy()[0]*255,y_bic)
        scaled_ycbcr_image = convert_rgb_to_ycbcr(
            resize_image_by_pil(input_image, scale))
        image = np.round(np.clip(convert_y_and_cbcr_to_rgb(output_y_image, scaled_ycbcr_image[:, :, 1:3]), 0, 255))
        ...
        image = np.round(output_y_image, 0, 255)
